...
func (s *RemoteRerankStage) request(ctx context.Context, query string, docs []string, subset []*model.SearchResult) ([]*model.SearchResult, error) {
	...
	// 构建去重外部分数映射 / Build deduplicated external score map
	externalScores := make(map[int]float64, len(parsed.Results)+len(parsed.Data))
	addScore := func(index int, score float64) {
		if index < 0 || index >= len(subset) {
			return
		}
		if _, exists := externalScores[index]; !exists {
			externalScores[index] = score
		}
	}
	for _, item := range parsed.Results {
		...
		addScore(item.Index, score)
	}
	for _, item := range parsed.Data {
		addScore(item.Index, item.Score)
	}
	...
	blended := blendScores(subset, externalScores, s.cfg.ScoreWeight)

	// 追加未参与评分的候选 / Append candidates not included in scoring
	seen := make(map[int]bool, len(externalScores))
	for idx := range externalScores {
		seen[idx] = true
	}
