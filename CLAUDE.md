...
#### 迁移文件必须按版本范围拆分 / Migration files must be split by version range

单个迁移文件禁止超过 500 行。按版本范围拆分为独立文件（如 `sqlite_migration_v0_v4.go`），主文件只保留调度逻辑和共享 helper。
> 经验：sqlite_migration.go 972 行拆为 4 文件（157+451+180+219），新增迁移不再担心文件超限。

#### 数据库模型必须使用 scanDest 模式 / DB models must use scanDest pattern

所有从数据库扫描的模型（Memory, Entity, Tag, Context, Document）必须使用 `scanDest` 结构体 + `scanFields()` + `toModel()` 三件套。禁止手写 `rows.Scan(&field1, &field2, ...)` 逐字段映射。
> 经验：Entity/Tag/Context/Document 各有 3-5 处手写 Scan，新增列必须改全部调用点，遗漏即 bug。

#### 错误分类函数必须集中在 store/errors.go / Error classifiers must live in store/errors.go

`IsColumnExistsError`、`IsUniqueConstraintError` 等 SQLite 错误分类函数统一放在 `store/errors.go` 并导出。禁止在各 store 文件中内联 `strings.Contains` 判断。
> 经验：同一个 UNIQUE 约束检查在 graph/tags/document/migration 4 个文件各写一遍，逻辑不一致。

#### API handler 必须使用 withIdentity 包装 / API handlers must use withIdentity wrapper

所有需要身份验证的 handler 必须使用 `withIdentity(h.Method)` 注册到路由，handler 签名为 `(c *gin.Context, identity *model.Identity)`。禁止在 handler 内部手写 `identity := requireIdentity(c)` + nil 检查。
> 经验：47 个 handler 各写 3 行身份提取模板，withIdentity 包装后减少 215 行样板代码。

#### Manager 方法必须单一职责 / Manager methods must have single responsibility

Manager 的公开方法（Create/Update/Delete 等）只做协调调度，具体逻辑（去重、嵌入解析、抽取）必须委托给专门的方法或文件。Create 超过 50 行时检查是否可以拆出子方法。
> 经验：Create 内联 35 行去重逻辑，抽取为 `dedupCheck()` 后 Create 减至 5 行调用。

#### 魔法数字必须抽为命名常量 / Magic numbers must be named constants

业务相关的数字（BM25 权重、阈值、限额）必须定义为 `const` 或配置项。同一数字出现 2 次以上时必须抽取。搜索方式：`grep -rn "具体数字" --include="*.go"` 确认无遗漏。
> 经验：BM25 权重 `10.0, 5.0, 3.0` 在 config/store/eval 三处硬编码，改一处忘两处。抽为 `DefaultBM25Content` 等常量后单点维护。
