...
// setHBConfig 设置全局 Heartbeat 配置 / Set global heartbeat config for test
func setHBConfig(enabled bool, contradictionEnabled bool, maxComp int) {
	config.AppConfig.Heartbeat = config.HeartbeatConfig{
		Enabled:              enabled,
...
func TestHeartbeat_Run_Disabled(t *testing.T) {
	setHBConfig(false, false, 0)
	stores, _ := setupHeartbeatStores(t)
	engine := heartbeat.NewEngine(stores.MemoryStore, stores.GraphStore, nil, nil)
...
func TestHeartbeat_ContradictionLLMError(t *testing.T) {
	setHBConfig(true, false, 5) // contradiction=false → LLM not called at all
	mockLLM := &hbMockLLM{err: assert.AnError}
	stores, _ := setupHeartbeatStores(t)
	engine := heartbeat.NewEngine(stores.MemoryStore, stores.GraphStore, nil, mockLLM)
	err := engine.Run(context.Background())
...
func TestHeartbeat_DecayAudit(t *testing.T) {
	...
	engine := heartbeat.NewEngine(stores.MemoryStore, nil, nil, nil)
	err := engine.Run(context.Background())
	assert.NoError(t, err)
}
