...
// TestLongMemEvalResolverFull 完整三层向量解析器评测（需要 Qdrant + Embedding API，无需 LLM）
func TestLongMemEvalResolverFull(t *testing.T) {
	apiKey := os.Getenv("EMBEDDING_API_KEY")
	if apiKey == "" {
		apiKey = os.Getenv("OPENAI_API_KEY")
	}
	if apiKey == "" {
		t.Skip("skip: EMBEDDING_API_KEY or OPENAI_API_KEY required for full resolver eval")
	}
	...
	entries, err := eval.LoadLongMemEval(datasetPath)
	require.NoError(t, err)
	t.Logf("Loaded %d questions (full resolver, 3-layer, max %d)", len(entries), maxQ)

	tmpDir := t.TempDir()
	report, err := eval.RunLongMemEvalResolverFull(context.Background(), entries, tmpDir, maxQ)
	require.NoError(t, err)

	eval.PrintReport(report)

	// 对比基线 / Compare baselines
	for _, name := range []string{"longmemeval-oracle-fts-v1", "longmemeval-oracle-resolver-v1"} {
		baseline, err := eval.LoadBaseline(name, "baselines")
		if err == nil {
			regressions := eval.CompareBaseline(report, baseline, eval.DefaultThresholds)
			eval.PrintComparison(report, baseline, regressions)
		}
	...
	require.NoError(t, eval.SaveBaseline(report, "longmemeval-oracle-resolver-full-v1", "baselines"))
	t.Logf("Full Resolver eval: HitRate %.1f%%, MRR %.3f, Duration %s",
		report.Metrics.HitRate, report.Metrics.MRR, report.Duration.Round(time.Second))
}
