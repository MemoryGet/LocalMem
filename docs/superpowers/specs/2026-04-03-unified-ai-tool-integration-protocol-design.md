...
1. 聚合本会话关键 observation
2. 调用 `iclude_ingest_conversation` 或等价汇总入口
3. 必要时生成 `session_summary`
...
3. `Stop` -> `iclude_ingest_conversation` 或 summary retain
...
4. 是否需要增加 `iclude_finalize_session` 作为比 `ingest_conversation` 更明确的协议动作
